...
HRESULT CLevel_GamePlay::Initialize()
{
	...
	CGameInstance::Get_Instance()->PlaySounds(TEXT("CreepyForest_vinyl_mastered.wav"), SOUND_BGM, 0.3f);
...
void CLevel_GamePlay::Set_BeargerAdd(_bool type)
{
	...
	{
		CGameInstance::Get_Instance()->StopSound(SOUND_BGM);
		CGameInstance::Get_Instance()->PlaySounds(TEXT("DST_BossFightNo3_V1.wav"), SOUND_BGM, 0.3f);
	}
}
	...
	if ((DAY_STATE)CDayCycle::Get_Instance()->Get_DayState() == DAY_STATE::DAY_NIGHT)
	{
		...
		if (m_iFirefliesCounter < m_iFirefliesMax)
		{
			if (m_fFirefliesTimer > .2f)
			{
				...
				tDecoDesc.vInitPosition = _float3(rand() % 55 + 15, 1.f, rand() % 30 + 10);
